...
bool TerrainShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"data/shaders/forward/Terrain.vs", L"data/shaders/forward/Terrain.ps");
	if (!result)
	{
		return false;
	}
...
bool TerrainShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMFLOAT4X4 worldMatrix, XMFLOAT4X4 viewMatrix,
							XMFLOAT4X4 projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* normalMap,
							ID3D11ShaderResourceView* normalMap2, ID3D11ShaderResourceView* normalMap3, XMFLOAT3 lightDirection, XMFLOAT4 diffuseColor)
{
	bool result;
...
void TerrainShader::OutputShaderErrorMessage(ID3DBlob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	char* compileErrors;
...
bool TerrainShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMFLOAT4X4 worldMatrix, XMFLOAT4X4 viewMatrix,
						 XMFLOAT4X4 projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* normalMap,
						 ID3D11ShaderResourceView* normalMap2, ID3D11ShaderResourceView* normalMap3, XMFLOAT3 lightDirection, XMFLOAT4 diffuseColor)
{
	HRESULT result;
	...
	dataPtr->world = XMMatrixTranspose(XMLoadFloat4x4(&worldMatrix));
	dataPtr->view = XMMatrixTranspose(XMLoadFloat4x4(&viewMatrix));
	dataPtr->projection = XMMatrixTranspose(XMLoadFloat4x4(&projectionMatrix));

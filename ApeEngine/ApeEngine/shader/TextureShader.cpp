...
bool TextureShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"data/shaders/forward/texture.vs", L"data/shaders/forward/texture.ps");
	if (!result)
	{
		return false;
	}
...
bool TextureShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, CXMMATRIX worldMatrix, CXMMATRIX viewMatrix,
...
bool TextureShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, CXMMATRIX worldMatrix, CXMMATRIX viewMatrix,
	CXMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture)
{
	HRESULT result;

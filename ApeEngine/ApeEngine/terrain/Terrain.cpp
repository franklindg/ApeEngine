...
bool Terrain::LoadTerrainCells(ID3D11Device* device)
{
	...
	m_TerrainCells = new TerrainCells[m_cellCount];
	if (!m_TerrainCells)
	{
		return false;
	}
...
bool Terrain::RenderCell(ID3D11DeviceContext* deviceContext, int cellID, Frustrum* frustrum)
{
	...
	result = frustrum->CheckRectangle2(maxWidth, maxHeight, maxDepth, minWidth, minHeight, minDepth);
	if (!result)
	{
